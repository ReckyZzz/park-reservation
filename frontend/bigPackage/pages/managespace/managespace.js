        ...
        this.fmap.on('mapClickNode', (event) => {
          ...
          switch (target.nodeType) {
            ...
            case fengmap.FMNodeType.MODEL:
              ...
              var orderId = {};
              var FID = target.FID;
              for(var i=0;i<this.data.reservedList.length;i++){
                if(this.data.reservedList[i].modelId == FID){
                  orderId = this.data.reservedList[i].orderId;
                }
              }
              //console.log(orderId);
              ...
              if(reserved === true){
                ...
                info = `停车位名称：${spaceName} 状态：未被预约`
              }
              this.selectedModel = target;
              
              //console.log(reserved);
              //console.log(reserved ? '取消该订单' : '设为被预约');
              var text = {};
              if(reserved){
                text = "取消订单"
              } 
                  ...
                  if(res.confirm){
                    if(reserved){
                      ...
                      wx.request({
                        ...
                        data:{
                          openId:'oeB3N5bdRFYW1LO_a9ERumaaKboE',
